class CanvasParser {
  ...
  addFileTreeButtons() {
    ...
    const existingButtons = document.querySelectorAll('#cp-filetree-md-btn, #cp-filetree-pdf-btn');
    ...
    mdBtn.className = 'btn';
    mdBtn.id = 'cp-filetree-md-btn';
    mdBtn.style.marginLeft = '5px';
    mdBtn.innerHTML = `🌳 MD Tree`;
    mdBtn.title = 'Download course structure as Markdown with links';
    
    mdBtn.addEventListener('click', (e) => {
      e.preventDefault();
      this.downloadMarkdownTree();
    });
  ...
  async downloadMarkdownTree() {
    const btn = document.getElementById('cp-filetree-md-btn');
    ...
    try {
      ...
      const treeText = this.generateFileTreeMarkdown();
      ...
      const filename = `${courseName}_ModulesTree_${dateStr}_${timeStr}.md`;
      
      
      this.downloadTextFile(treeText, filename);
      
      btn.innerHTML = '✅ MD tree downloaded';
    } catch (error) {
      console.error('Failed to generate MD tree:', error);
      btn.innerHTML = '❌ MD tree generation failed';
    }
